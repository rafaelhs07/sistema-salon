...
export default function CuentaPagarDetalleClient({ cuenta }: { cuenta: CuentaPagarDetalle }) {
    ...
    return (
        <div className="mx-auto max-w-[1600px] space-y-6">
            ...
            <section className="rounded-[32px] bg-[#26332F] p-6 text-white shadow-[0_18px_50px_rgba(36,48,44,0.16)] sm:p-8">
                ...
                <div className="mt-5 flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            <div className="flex flex-wrap items-center gap-2">
                                <h1 className="text-3xl font-bold sm:text-4xl">{cuenta.proveedorNombre}</h1>
                    ...
                    {permiteAbono && (
                        <div className="flex flex-wrap gap-2">
                            <button onClick={() => abrirPago(false)} className="inline-flex h-12 items-center gap-2 rounded-xl border border-white/20 bg-white/10 px-5 font-bold">
                                <WalletCards className="h-5 w-5" />
                                Registrar abono
                            </button>
                            <button onClick={() => abrirPago(true)} className="inline-flex h-12 items-center gap-2 rounded-xl bg-[#DCE7E2] px-5 font-bold text-[#26332F]">
            ...
            <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
                <div className="rounded-[28px] border border-[#E3E7E4] bg-white p-5 sm:p-6">
                    <h2 className="text-lg font-bold text-[#24302C]">Historial de pagos</h2>
                    <p className="mt-1 text-sm text-[#6B756F]">Abonos realizados a esta cuenta.</p>
                    ...
                    {cuenta.abonos.length === 0 ? (
                        ...
                        <div className="mt-5 space-y-3">
                            {cuenta.abonos.map((a) => (
                                <article key={a.id} className="rounded-2xl border border-[#E3E7E4] bg-[#FBFCFA] p-4">
                                    <div className="flex items-center justify-between gap-4">
                                        <div>
                                            <p className="font-bold text-[#24302C]">{metodoNombre(a.metodoPago)}</p>
                                            <p className="mt-1 text-xs text-[#76817B]">{fechaHora(a.fechaAbono)}</p>
                                            {a.referencia && <p className="mt-1 text-xs text-[#6B756F]">Ref. {a.referencia}</p>}
                                        </div>
                                        <p className="text-lg font-bold text-[#527064]">{dinero(a.monto)}</p>
                                    </div>
                                </article>
                            ))}
                        </div>
                ...
                <aside className="space-y-5">
                    <div className="rounded-[28px] border border-[#E3E7E4] bg-white p-5">
                        <h2 className="font-bold text-[#24302C]">Proveedor</h2>
                    ...
                    <div className="rounded-[28px] border border-[#E3E7E4] bg-white p-5">
                        <h2 className="font-bold text-[#24302C]">Compra asociada</h2>
                        <p className="mt-3 text-sm font-semibold text-[#33413B]">{cuenta.codigoCompra}</p>
                        <Link href={`/compras/${cuenta.compraId}`} className="mt-4 inline-flex h-10 w-full items-center justify-center rounded-xl bg-[#EEF2EF] text-sm font-bold text-[#43524B]">
                            Ver compra
                        </Link>
                    </div>
                </aside>
            ...
            {abierto && (
                <div className="fixed inset-0 z-[90] flex items-center justify-center bg-[#26332F]/50 p-4 backdrop-blur-sm">
                    ...
                    <div className="relative z-10 w-full max-w-xl rounded-[28px] bg-white p-6 shadow-2xl">
                        <div className="flex items-start justify-between">
                            <div>
                                <p className="text-sm font-semibold text-[#6F8F83]">Pago a proveedor</p>
                                <h2 className="mt-1 text-xl font-bold text-[#24302C]">Registrar abono</h2>
                            </div>
                            <button onClick={() => setAbierto(false)} disabled={procesando} className="rounded-xl p-2 text-[#6B756F] hover:bg-[#EEF2EF]">
                                <X className="h-5 w-5" />
                            </button>
                        </div>
                        ...
                        <div className="mt-5 rounded-2xl bg-[#F0F5F2] p-4">
                            ...
                            <p className="mt-1 text-2xl font-bold text-[#24302C]">{dinero(cuenta.saldoPendiente)}</p>
                        </div>
                        ...
                        <div className="mt-5 grid gap-4 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <select value={metodo} onChange={(e) => setMetodo(e.target.value as MetodoPagoProveedor)} className="h-11 w-full rounded-xl border border-[#D4DAD6] bg-white px-4 text-sm">
                            ...
                            <div className="sm:col-span-2">
                                ...
                                <textarea rows={3} value={observaciones} onChange={(e) => setObservaciones(e.target.value)} className="w-full rounded-xl border border-[#D4DAD6] px-4 py-3 text-sm" />
                            </div>
                        </div>
                        ...
                        <button
                            ...
                            className="mt-6 inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#6F8F83] px-5 text-sm font-bold text-white disabled:opacity-50"
                        >
...
function Resumen({ titulo, valor, icono: Icono }: { titulo: string; valor: string; icono: React.ComponentType<{ className?: string }> }) {
    return <article className="rounded-3xl border border-[#E3E7E4] bg-white p-5"><p className="text-sm text-[#6B756F]">{titulo}</p><div className="mt-3 flex items-center justify-between gap-3"><p className="text-2xl font-bold text-[#24302C]">{valor}</p><Icono className="h-5 w-5 text-[#527064]" /></div></article>;
}
...
function Campo({ etiqueta, valor, cambiar, tipo = "text" }: { etiqueta: string; valor: string; cambiar: (v: string) => void; tipo?: string }) {
    return <div><label className="mb-2 block text-xs font-bold text-[#52605A]">{etiqueta}</label><input type={tipo} min={tipo === "number" ? "0.01" : undefined} step={tipo === "number" ? "0.01" : undefined} value={valor} onChange={(e) => cambiar(e.target.value)} className="h-11 w-full rounded-xl border border-[#D4DAD6] px-4 text-sm" /></div>;
}
